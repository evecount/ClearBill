...
import { Plus, Trash2, Wand2, ArrowLeft, Send, Scale, Loader2, FileText, BarChart3, Info, ShieldCheck } from "lucide-react"
  ...
  const handleBenchmarkPrice = async (itemId: string, description: string) => {
    ...
    try {
      ...
      toast({ 
        ...
        description: `Suggested rate: $${result.suggestedRateRange.min} - $${result.suggestedRateRange.max}` 
      })
    } catch (error) {
      toast({ title: "Benchmark Error", description: "Could not audit market rates." })
    } finally {
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="md:col-span-2 space-y-6">
          <Card className="border-none shadow-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-black">Line Items</Label>
                  ...
                  <div key={item.id} className="grid gap-3 items-end sm:grid-cols-12 border-b pb-4">
                    <div className="sm:col-span-6 space-y-1.5">
                      ...
                      <div className="relative">
                        <Input 
                          placeholder="What outcome was delivered?" 
                ...
                <Button variant="outline" size="sm" onClick={addItem} className="w-full border-dashed rounded-xl h-11">
                  <Plus className="size-4 mr-2" /> Add Line Item
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-accent/20 bg-accent/5">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-lg flex items-center gap-2">
                  <Scale className="size-5 text-accent" /> Strategic Agreement
                </CardTitle>
                <p className="text-xs text-muted-foreground mt-1">Include a professional outcome-based agreement.</p>
              </div>
              <Button 
                variant="outline" 
                size="sm" 
            ...
            <CardContent>
              <Textarea 
                placeholder="The agreement will appear here..." 
                className="min-h-[150px] rounded-xl bg-white/50 border-accent/10 text-sm"
        ...
        <div className="space-y-6">
          <Card className="shadow-lg border-none">
            ...
            <CardFooter>
              <Button className="w-full h-14 bg-accent hover:bg-accent/90 rounded-xl text-lg shadow-lg" onClick={handleCreate} disabled={loading}>
          ...
          <div className="p-4 bg-slate-900 text-white rounded-2xl space-y-2 text-center">
             <div className="flex items-center justify-center gap-2 mb-2">
               ...
               <span className="text-[10px] font-black uppercase tracking-widest">Fact Verification</span>
             </div>
             <p className="text-[10px] text-slate-400 leading-relaxed italic">
               "Billed by {org?.name || 'Your Business'}. Based in {org?.addressLine1 || 'Your Location'}."
             </p>
          </div>
        </div>
      </div>
